...
import { ConsultaFicha, ItemComConsulta } from "@/lib/types";
import { Breadcrumb } from "@/components/Breadcrumb";
import { PetAvatar } from "@/components/PetAvatar";
import { PetInfoBadges } from "@/components/PetInfoBadges";

interface TutorData {
  pets: { id: number; nome: string; especie: string; raca?: string; foto_url?: string; nome_tutor: string; telefone?: string; data_nascimento?: string; sexo?: string }[];
  nome_tutor: string;
...
function TutorContent() {
  ...
  useEffect(() => {
    ...
    const q = telefone ? `nome=${encodeURIComponent(nome)}&telefone=${encodeURIComponent(telefone)}` : `nome=${encodeURIComponent(nome)}`;
    api<TutorData>(`${API_BASE}/tutores/ficha?${q}`).then((r) => {
  ...
  if (!nome.trim()) {
    return (
      <section className="max-w-6xl">
        <Breadcrumb crumbs={[{ href: "/", label: "Home" }, { label: "Tutor" }]} />
        <p className={CN.listEmpty}>Informe o nome do tutor na URL: /tutor?nome=Nome do Tutor</p>
      </section>
    );
  }

  if (loading) return <p className={CN.listEmpty}>Carregando...</p>;
  if (!data) return <p className={CN.listEmpty}>Tutor não encontrado.</p>;

  const { pets, nome_tutor, consultas } = data;
  ...
  return (
    <section className="max-w-6xl">
      <Breadcrumb crumbs={[{ href: "/", label: "Home" }, { href: "/?s=pets", label: "Pets" }, { label: nome_tutor }]} />
      ...
      <div className="flex flex-col sm:flex-row gap-8 p-6 lg:p-8 rounded-[var(--radius-xl)] bg-white border border-slate-100/80 shadow-[var(--card-shadow)] mb-8">
        <div className="w-28 h-28 rounded-2xl bg-gradient-to-br from-[var(--primary-muted)] to-[var(--primary-light)]/50 flex items-center justify-center flex-shrink-0 shadow-inner">
          <span className="material-icons-round text-4xl text-[var(--primary)]">person</span>
        </div>
        <div className="flex-1 min-w-0">
          <h1 className="text-2xl font-bold text-[var(--text)] mb-1 tracking-tight">{nome_tutor}</h1>
          {data.telefone && (
            <a href={`tel:${data.telefone.replace(/\D/g, "")}`} className="flex items-center gap-2 text-[var(--primary)] hover:underline text-[0.875rem] mt-2">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
        <div className="p-5 rounded-[var(--radius-lg)] bg-white border border-slate-100/80 shadow-[var(--shadow)] hover:shadow-[var(--shadow-md)] transition-shadow duration-200">
          <div className="flex items-center gap-2 text-[var(--text-muted)] text-[0.75rem] font-medium mb-1">
            <span className="material-icons-round text-base">pets</span> Pets
          </div>
          <div className="text-2xl font-bold text-[var(--primary)]">{pets.length}</div>
        </div>
        <div className="p-5 rounded-[var(--radius-lg)] bg-white border border-slate-100/80 shadow-[var(--shadow)] hover:shadow-[var(--shadow-md)] transition-shadow duration-200">
          <div className="flex items-center gap-2 text-[var(--text-muted)] text-[0.75rem] font-medium mb-1">
      ...
      <div className="rounded-[var(--radius-lg)] bg-white border border-slate-100/80 shadow-[var(--shadow)] overflow-hidden mb-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 p-5">
          {!pets.length && <p className={`${CN.listEmpty} col-span-full`}>Nenhum pet.</p>}
          {pets.map((p) => (
            <Link key={p.id} href={`/pet/${p.id}`} className="flex flex-col rounded-[var(--radius)] border-l-[3px] border-l-[var(--primary)] bg-slate-50/80 hover:bg-[var(--primary-subtle)] p-4 transition-colors">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <PetInfoBadges data_nascimento={p.data_nascimento} sexo={p.sexo} especie={p.especie} showEspecie className="mt-1" />
                </div>
              </div>
          ...
          <div className="divide-y divide-slate-100">
            ...
            {consultas.map((c) => (
              <Link key={c.id} href={`/pet/${c.pet_id}/consulta/${c.id}`} className="flex items-center gap-3 px-4 py-3 hover:bg-[var(--primary-subtle)] transition-colors">
                <PetAvatar fotoUrl={c.pet_foto_url} especie={c.pet_especie ?? ""} nome={c.pet_nome ?? ""} size="xs" className="shrink-0" />
                <div className="flex-1 min-w-0">
...
export default function TutorPage() {
  return (
    <Suspense fallback={<p className={CN.listEmpty}>Carregando...</p>}>
      <TutorContent />
    </Suspense>
  );
}
